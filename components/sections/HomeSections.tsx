    ...
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-brand-primary">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-3xl">
            ...
            <motion.div variants={fadeInVariant} className="flex flex-wrap gap-4 mb-12">
              <Button size="lg" className="gap-2">{t('enrolNow')} <ArrowRight className="w-5 h-5" /></Button>
              <Button size="lg" variant="ghost">{t('exploreCourses')}</Button>
            </motion.div>
            
            <motion.div variants={fadeInVariant} className="flex flex-wrap gap-6 items-center text-sm font-medium text-brand-text-muted">
              <div className="flex items-center gap-2"><Users className="w-4 h-4 text-brand-accent"/> {t('allAges')}</div>
              <div className="w-1 h-1 rounded-full bg-brand-border hidden sm:block"/>
              <div className="flex items-center gap-2"><Users className="w-4 h-4 text-brand-accent"/> {t('maleFemale')}</div>
              <div className="w-1 h-1 rounded-full bg-brand-border hidden sm:block"/>
              <div className="flex items-center gap-2"><Globe className="w-4 h-4 text-brand-accent"/> {t('worldwide')}</div>
            </motion.div>
          </motion.div>
      ...
      <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-none">
        <svg className="relative block w-full h-[50px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
          <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V120H0V95.8C59.71,118.08,130.83,110.2,192.39,100.8,236.4,94.23,279.7,80.9,321.39,56.44Z" fill="var(--bg-section)"></path>
        </svg>
      </div>
    </section>
  );
    ...
    <section className="bg-brand-section py-20 relative z-20">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-brand-border">
          {[
            { label: t('students'), value: '500+' },
            { label: t('islamic'), value: '9+' },
            { label: t('academic'), value: '3' },
            { label: t('teachers'), value: '20+' }
          ].map((stat, i) => (
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
             <motion.div initial="hidden" whileInView="visible" viewport={{once: true}} variants={staggerContainer}>
                ...
                <motion.div variants={fadeInVariant}>
                   <Button variant="link" className="px-0 text-brand-accent text-lg">{t('learnMore')}</Button>
                </motion.div>
             </motion.div>
             ...
             <motion.div initial={{opacity:0, x:30}} whileInView={{opacity:1, x:0}} viewport={{once:true}} transition={{duration:0.8}} className="relative">
                ...
                <div className="absolute -top-4 -left-4 w-12 h-12 border-t-2 border-l-2 border-brand-accent rtl:-right-4 rtl:-left-auto rtl:border-l-0 rtl:border-r-2" />
                <div className="absolute -bottom-4 -right-4 w-12 h-12 border-b-2 border-r-2 border-brand-accent rtl:-left-4 rtl:-right-auto rtl:border-r-0 rtl:border-l-2" />
             </motion.div>
          </div>
       </div>
    </section>
...
export function ProgrammesSection() {
  const t = useTranslations('HomePage.programmes');
  return (
             ...
             <Card className="h-full bg-brand-green/20 border-brand-accent/30 overflow-hidden relative group">
               ...
               <CardContent className="p-10 relative z-10 flex flex-col h-full">
                 ...
                 <div className="mt-auto">
                   <Button variant="link" className="text-brand-accent px-0 hover:text-brand-amber">{t('exploreIslamic')}</Button>
                 </div>
               </CardContent>
             </Card>
          </motion.div>
          ...
          <motion.div initial={{opacity:0, y:30}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{delay:0.2}}>
             <Card className="h-full bg-brand-primary border-brand-accent/20 overflow-hidden relative group">
               <CardContent className="p-10 relative z-10 flex flex-col h-full">
                 ...
                 <div className="mt-auto">
                   <Button variant="link" className="text-brand-accent px-0 hover:text-brand-amber">{t('exploreAcademic')}</Button>
                 </div>
               </CardContent>
             </Card>
          </motion.div>
          ...
          <Card className="bg-brand-primary/50 border-brand-accent/20">
            <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <Button asChild variant="ghost">
                <a href="https://rawdahkids.org" target="_blank" rel="noreferrer">{t('visitRawdah')}</a>
              </Button>
            </CardContent>
          </Card>
        </motion.div>
...
export function FeaturesSection() {
  ...
  const features = [
    ...
    { icon: Users, title: t('f2Title'), desc: t('f2Desc') },
    { icon: Users, title: t('f3Title'), desc: t('f3Desc') },
    { icon: Clock, title: t('f4Title'), desc: t('f4Desc') },
    ...
    <section className="py-24 relative overflow-hidden bg-brand-primary border-t border-brand-border">
      ...
      <div className="container mx-auto px-4 relative z-10 text-center max-w-3xl">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <Button size="lg" variant="ghost">{t('contact')}</Button>
        </div>
      </div>
    </section>
  );
